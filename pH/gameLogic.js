...
var meterDisplay = JSON.parse(JSON.stringify(DEFAULT_METER_DISPLAY));
...
function startStep(step) {
    ...
    if (step.id == "readMeter") {
        // Show the input pH box
        showPHInput();
        $("#inputPHInput").focus();
    }
    if (step.id == "selectStorageSolution") {
        $("#jarSwitchBox").addClass("anim_switchJars");
        setTimeout(function () {
...
function refreshHighScores() {
    ...
    //        $("#scoreBox3").text("");
    //    }
    // Temporary: Display if the game mode has been completed
    for (var i = 0; i < 3; i++) {
...
function lockModes() {
    ...
    if (studentData.unlocks[0] || UNLOCK_EVERYTHING) {
        // Unlock
        $("#endOptionDescText2").html("Find the pH of a solution. No hints!");
        $("#endOption2").removeClass("endOptionLocked");
        $("#endOption2").addClass("endOptionUnlocked");
    } else {
        // Lock
        $("#endOptionDescText2").html("Locked! <br> Clear Practice mode to unlock.");
    ...
    if ((studentData.unlocks[1] || UNLOCK_EVERYTHING) && false) {
        // Unlock
        $("#endOptionDescText3").html("Coming soon!");
        $("#endOption3").removeClass("endOptionLocked");
        $("#endOption3").addClass("endOptionUnlocked");
    } else {
        // Lock
        $("#endOptionDescText3").html("Locked! <br> Challenge+ mode is coming soon!");
...
function tryElectrodeUp() {
    if (game.props.mode != 1 || game.getCurrentStep().id == "liftElectrode") {
        ...
        if (game.getCurrentStep().id == "liftElectrode") {
            ...
            // Fail game
            electrodeUp();
            failGame("raised the electrode", "");
        }
    }
}

function tryElectrodeDown() {
    if (game.props.mode != 1 || game.getCurrentStep().id == "lowerElectrode" || game.getCurrentStep().id == "storeElectrode") {
        enableClicks(false);
        if (game.getCurrentStep().id == "lowerElectrode") {
            ...
            electrodeDown();
        } else if (game.getCurrentStep().id == "storeElectrode") {
            electrodeDown();
            game.nextStep();
        } else {
            // Fail game
            electrodeDown();
            failGame("lowered the electrode", "");
        }
    }
}
...
function tryRinseElectrode() {
    if (game.props.mode != 1 || game.getCurrentStep().id == "rinseElectrode") {
        ...
        if (game.getCurrentStep().id == "rinseElectrode") {
            ...
            // Fail game
            failGame("tried to rinse the electrode", "");
        }
    }
}
...
function tryWipeElectrode() {
    if (game.props.mode != 1 || game.getCurrentStep().id == "wipeElectrode") {
        ...
        if (game.getCurrentStep().id == "wipeElectrode") {
            ...
            // Fail game
            failGame("tried to blot the electrode", "");
        }
    }
}
...
function tryTakeSmallJar(i) {
    if (game.props.mode != 1 || (game.getCurrentStep().id == "selectBuffer" || game.getCurrentStep().id == "selectSolution" || game.getCurrentStep().id == "selectStorageSolution")) {
        if (game.getCurrentStep().id == "selectBuffer" && buffersUsed[i - 2] == false) {
            ...
            takeSmallJar(i);
        } else if (i == 1 && game.getCurrentStep().id == "selectStorageSolution") {
            takeSmallJar(i);
        } else if (game.props.mode != 1) {
            // Fail game
            failGame("tried to take the wrong jar", "");
        } else {
...
function tryStirJar() {
    if (game.props.mode != 1 || game.getCurrentStep().id == "stirSolution") {
        ...
        if (game.getCurrentStep().id == "stirSolution") {
            ...
            // Fail game
            failGame("tried to swirl the solution", "");
        }
    }
}
...
function tryOpenElectrodeHole() {
    if (game.props.mode != 1 || game.getCurrentStep().id == "openElectrodeHole" || game.getCurrentStep().id == "closeElectrodeHole") {
        if (game.getCurrentStep().id == "openElectrodeHole") {
            openElectrodeHole();
        } else if (game.getCurrentStep().id == "closeElectrodeHole") {
            closeElectrodeHole();
        } else {
            // Click electrode
            $("#electrode").click();
...
function closeElectrodeHole() {
    $("#holeBGClear").removeClass("anim_openElectrodeHole");
    $("#holeBGClear").addClass("anim_closeElectrodeHole");
    enableClicks(false);
    setTimeout(function () {
        game.nextStep();
    }, 500);
...
function showPHInput() {
    $("#inputPHBox").removeClass("anim_inputPHOut");
    $("#inputPHBox").addClass("anim_inputPHIn");
}

function hidePHInput() {
    $("#inputPHBox").removeClass("anim_inputPHIn");
    $("#inputPHBox").addClass("anim_inputPHOut");
}